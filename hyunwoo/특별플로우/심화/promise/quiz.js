...
const changeSize = async () => {
  $slide.classList.remove("min-size");
  $slide.classList.add("max-size");
  const percentage = await prom(15);
  $slide.innerText = `이번주 목표까지 ${percentage}% 남았습니다`;
};
window.onload = () => {
  setTimeout(() => {
    changeSize();
  }, 500); // 이렇게 하지 않으면 효과가 잘 안 보임.
};
